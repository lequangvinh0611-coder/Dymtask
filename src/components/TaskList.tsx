    ...
    <div className="flex-1 flex flex-col min-h-0 bg-white shadow-sm overflow-hidden">
      <div className="px-4 py-1.5 border-b border-slate-100 flex items-center bg-white shrink-0 overflow-x-auto no-scrollbar">
        ...
        <div className="ml-auto flex items-center gap-2">
          {showCreate && (
             <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-1.5 h-7 px-3 bg-indigo-600 hover:bg-indigo-700 transition-colors text-white rounded-lg text-[10px] font-bold uppercase tracking-wider">
                <Plus className="w-3 h-3" /> <span>Create Task</span>
             </button>
          )}
        </div>
      </div>
